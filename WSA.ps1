<#
.SYNOPSIS
    Windows Security Audit Tool — By Kuanyshgali Ishimbayev
    Аудит безопасности Windows: пароли, учётные записи, firewall, службы, сетевые протоколы,
    обновления, защита (Defender/Credential Guard/LSA/BitLocker) и аудит/логирование.
    Формирует HTML-отчёт (с фильтрами и поиском) и опционально JSON/CSV с конкретными шагами устранения.

.DESCRIPTION
    Для каждой проблемной проверки (Warning/Fail) отчёт содержит:
      - в чём риск
      - конкретные команды / шаги для устранения
    Каждая проверка выполняется в изолированном блоке — сбой одной проверки не прерывает весь аудит
    и фиксируется в отчёте как отдельная запись "Проверка не выполнена".

.PARAMETER OutputPath
    Путь к итоговому HTML-файлу. По умолчанию создаётся рядом со скриптом с меткой времени.

.PARAMETER ExportJson
    Дополнительно сохранить результаты в формате JSON (для интеграции с SIEM/скриптами).

.PARAMETER ExportCsv
    Дополнительно сохранить результаты в формате CSV (для Excel/сводных таблиц).

.PARAMETER NoLaunch
    Не открывать HTML-отчёт автоматически после генерации.

.PARAMETER Quiet
    Не выводить прогресс в консоль (полезно при запуске по расписанию).

.PARAMETER NoExitCode
    Не устанавливать код завершения процесса по результатам (по умолчанию скрипт завершается
    с кодом 1, если найдена хотя бы одна проблема статуса Fail — удобно для CI/мониторинга).

.PARAMETER NoElevate
    Не пытаться автоматически перезапуститься с правами администратора через UAC.
    Часть проверок в этом случае будет недоступна и отмечена как "Info".

.PARAMETER Categories
    Список категорий для проверки (например: "Firewall","Сеть"). Если указан — интерактивное
    меню не показывается. Категории: Система, Пароли, Учётные записи, Firewall, Сеть,
    Службы, Обновления, Защита, Аудит и логирование.

.PARAMETER NoMenu
    Не показывать интерактивное меню выбора — сразу выполнить полный аудит (или по списку
    из -Categories, если указан). Полезно при запуске по расписанию или в CI.

.EXAMPLE
    .\Windows-Security-Audit.ps1
    Запуск с интерактивным меню — выбор категорий и параметров экспорта перед стартом.
.EXAMPLE
    .\Windows-Security-Audit.ps1 -Categories "Firewall","Сеть" -NoMenu
.EXAMPLE
    .\Windows-Security-Audit.ps1 -NoMenu -ExportJson -ExportCsv -NoLaunch -Quiet
.NOTES
    Скрипт сам запросит повышение прав через UAC при запуске без администраторских прав.
    Для запуска также можно использовать прилагаемый Run-SecurityAudit.cmd — он обходит
    ограничения политики выполнения (ExecutionPolicy) и запрашивает права администратора
    одним двойным щелчком.
#>
