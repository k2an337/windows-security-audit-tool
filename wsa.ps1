...
.EXAMPLE
    .\Windows-Security-Audit.ps1 -ExportJson -ExportCsv -NoLaunch -Quiet
.NOTES
...
foreach ($cat in ($results | Select-Object -ExpandProperty Category -Unique)) {
    ...
    foreach ($r in $catResults) {
        ...
        $remBlock = if ($r.Remediation) { "<div class='rem'><b>✅ Как устранить:</b><div class='rem-code'>$(Format-Remediation (Format-Html $r.Remediation))</div></div>" } else { "" }
        $rows += @"
...
"@
    ...
    $categoriesHtml += @"
    <details class="category"$openAttr>
        <summary><h2>$(Format-Html $cat) $badgesHtml</h2></summary>
        <table>
